...
export default function TopThree({ rows }: { rows: StandingRow[] }) {
  return (
    <div className="grid gap-4 md:grid-cols-3">
      {rows.slice(0, 3).map((row) => (
        <Card key={row.entryId} className="transition-all duration-300 hover:-translate-y-2 hover:shadow-xl">
          <CardHeader>
            <CardTitle className="flex justify-between">
              <span>{RankBadge({ rank: row.rank })}</span>
